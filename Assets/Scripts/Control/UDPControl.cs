...
public class UDPControl : MonoBehaviour
{
    ...
    private vMPLMovementArbiter.MovementState movement_class = vMPLMovementArbiter.MovementState.Invalid;

    private float [] arm_angles = new float[NUM_MPL_ARM_ANGLES];
    private float [] finger_angles = new float[NUM_MPL_FINGER_ANGLES];
    
    // synchronization variables
    bool update_class = false;
    ...
    void Update() {
        if ( update_class ) {
            arbiter.SetMovementState( movement_class );
            update_class = false;
        }
            ...
            try {
                ...
                switch( cmd ) {
                    case 0x6a: // (j)oint angles
                        ...
                        // check if we received a movement class
                        movement_class = (vMPLMovementArbiter.MovementState) System.BitConverter.ToInt32( receiveBytes, idx );
                        idx += 4;
                
                        if ( movement_class == vMPLMovementArbiter.MovementState.Invalid ) {
